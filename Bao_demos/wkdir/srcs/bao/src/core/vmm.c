...
spinlock_t g_mapping_lock;
volatile size_t g_vms_mapped_count = 0;
volatile bool g_vm0_has_started = false;
...
void vmm_init()
{
    ...
    if (vmm_assign_vcpu(&master, &vm_id)) {
        ...
        // Barreira - Etapa 1: Espera que todas as VMs terminem o mapeamento.
        while(g_vms_mapped_count < config.vmlist_size) {
            asm volatile("yield"); 
        }

        // Barreira - Etapa 2: Espera pela VM 0, se necessário.
        if (vm_id == 0) {
            if(master) {
                g_vm0_has_started = true;
            }
        } else {
            while(!g_vm0_has_started) {
                asm volatile("yield");
            }
        }
